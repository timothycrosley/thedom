...
class JS(object):
    ...
    def visit(self, node, scope=None):
        try:
            ...
            raise JSError("syntax not supported (%s) - %s" % (node, self.name(node)))
    ...
    def visit_FunctionDef(self, node):
        ...
        if node.decorator_list:
            if len(node.decorator_list) == 1 and \
                    ...
                    node.decorator_list[0].id == "Convert":
                is_javascript = True # this is our own decorator
            elif len(node.decorator_list) == 1 and \
                    isinstance(node.decorator_list[0], ast.Attribute) and \
                    node.decorator_list[0].attr == "Convert":
                is_javascript = True # this is our own decorator
    ...
    def visit_Call(self, node):
        ...
        if node.keywords:
            ...
            if func == "js" and len(node.args) == 1:
                return str(node.args[0].s)
